...
public interface MangaRepository extends JpaRepository<Manga, Integer> {
    ...
    @Query("SELECT m FROM Manga m JOIN m.genres g WHERE g.name IN :genreNames GROUP BY m.id HAVING COUNT(DISTINCT g.name) = :size")
    List<Manga> findAllByGenreNames(@Param("genreNames") List<String> genreNames, @Param("size") long size);
